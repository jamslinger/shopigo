...
func (c *client) GetWebhooks(ctx context.Context) ([]*Webhook, error) {
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, c.config.Endpoint(c.sess.Shop, c.version, "/webhooks.json"), nil)
	if err != nil {
		return nil, err
	}
...
func (c *client) RegisterWebhook(ctx context.Context, wh *Webhook) (id int, err error) {
	...
	if id == 0 {
		req, err = http.NewRequestWithContext(ctx, http.MethodPost,
			c.config.Endpoint(c.sess.Shop, c.version, "/webhooks.json"), bytes.NewBuffer(body))
		if err != nil {
			return 0, err
		}
	} else {
		req, err = http.NewRequestWithContext(ctx, http.MethodPut,
			c.config.Endpoint(c.sess.Shop, c.version, fmt.Sprintf("/webhooks/%d.json", id)), bytes.NewBuffer(body))
		if err != nil {
			return 0, err
		}
	}
...
func (c *client) DeleteWebhook(ctx context.Context, id int) error {
	req, err := http.NewRequestWithContext(ctx, http.MethodDelete,
		c.config.Endpoint(c.sess.Shop, c.version, fmt.Sprintf("/webhooks/%d.json", id)), nil)
	if err != nil {
		return err
	}
	req.Header.Add("Content-Type", "application/json")
